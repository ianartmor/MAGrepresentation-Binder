...
df_plotly <- as.data.frame(otu_table(tara_norm_physeq))
x <- colnames(df_plotly)
y <- rownames(df_plotly)
plot_ly(x=x,y=y,z =df_plotly, type = "heatmap")
```
...
plot_ordination(tara_norm_physeq, tara_norm.ord, color = "province")+ facet_wrap(~size_fraction)+theme_minimal()

bact <- subset_samples(tara_norm_physeq, size_fraction == "bact")
bact.ord <- ordinate(bact, method = "PCoA", distance = "bray")
plot_ordination(bact, bact.ord, color = "depth")+theme_minimal()
...
abundplot <- ggplot(top_bact_df, aes(x=province, y= Abundance, color = depth))+facet_wrap(~OTU, scales = "free_y") + geom_jitter()+theme_minimal()+ labs(y = "Normalized MAG Abundance") + 
